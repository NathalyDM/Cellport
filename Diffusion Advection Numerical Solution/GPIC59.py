...
sheet_name_golgi='GPICD59 Golgi Exit'
sheet_name_ER='GPICD59 ER Exit'

plt.close('all')
###############  IMPORTING DATAFRAME   ###############
df_golgi = pd.read_excel(r'C:/Users/utraf/Documents/Nathaly/Parisa/Intracisternal Golgi Model/Parisa Send to Nathaly_21092022.xlsx', sheet_name=sheet_name_golgi)
df_golgi.dropna(axis=0,inplace=True)

df_ER = pd.read_excel(r'C:/Users/utraf/Documents/Nathaly/Parisa/Intracisternal Golgi Model/Parisa Send to Nathaly_21092022.xlsx', sheet_name=sheet_name_ER)
...
###############  FILTERING DATA   ###############
ER_time_array,ER_array,ER_xlim_time=filtering_ER(ER_time,ER_MFI,0,int((20-8*step)/step),0.58)
Golgi_time_array,Golgi_array,Golgi_xlim_time=filtering_Golgi(golgi_time,golgi_MFI,int((10-8*step)/step),int((35-8*step)/step),0.5)
...
for protein_mfi in ER_array:
    ...
    # [D,V,r,kL, kR, dt, Dt]
    df2 = pd.DataFrame([{'System':'GPICD59','Organelle':'Golgi','Cell':cell,'D(min-1)':best_specimen[0],'V(min-1)':best_specimen[1],
...
with open("GPICD59/ER_time_array", "wb") as fp:
    pickle.dump(ER_time_array, fp)

with open("GPICD59/ER_MFI", "wb") as fp:
    pickle.dump(ER_array, fp)

with open("GPICD59/Golgi_time_array", "wb") as fp:
    pickle.dump(Golgi_time_array, fp)

with open("GPICD59/Golgi_MFI", "wb") as fp:
    pickle.dump(Golgi_array, fp)

df_solutions_Golgi.to_csv('GPICD59/GPICD59 Golgi Exit.csv')
df_solutions_RE.to_csv('GPICD59/GPICD59 RE Exit.csv')
