...
sheet_name_golgi='GPIFRA Golgi Exit'
sheet_name_ER='GPIFRA ER Exit'
...
for protein_mfi in ER_array:
    ...
    # [D,V,r,kL, kR, dt, Dt]
    df2 = pd.DataFrame([{'System':'GPIFRA','Organelle':'Golgi','Cell':cell,'D(min-1)':best_specimen[0],'V(min-1)':best_specimen[1],
...
with open("GPIFRA/ER_time_array", "wb") as fp:
    pickle.dump(ER_time_array, fp)

with open("GPIFRA/ER_MFI", "wb") as fp:
    pickle.dump(ER_array, fp)

with open("GPIFRA/Golgi_time_array", "wb") as fp:
    pickle.dump(Golgi_time_array, fp)

with open("GPIFRA/Golgi_MFI", "wb") as fp:
    pickle.dump(Golgi_array, fp)

df_solutions_Golgi.to_csv('GPIFRA/GPIFRA Golgi Exit.csv')
df_solutions_RE.to_csv('GPIFRA/GPIFRA RE Exit.csv')
